...
public class LeitorDeArquivos { // public pq uso eles em outra clase
  ...
  public HashMap<String, Capitulo> lerCapitulos(String path, Map<String, Personagem> personagens) {
    ...
    try {
      ...
      // loop se repetirá até existir uma nova linha a ser lida.
      {
        if (linha.equals("CAPITULO") || linha.equals("CAPITULO_COM_IMAGEM"))// se minha linha for igual a "capitulo",
                                                                            // entre no método ler capitulo
        {
          ...
          if (linha.equals("CAPITULO")) {
            capitulos.put(id, new Capitulo(personagens, escaneadorDoArquivo));// aqui ele valores aos dicionario se já
                                                                              // existisse são substituídos, ou seja
          } else if (linha.equals("CAPITULO_COM_IMAGEM")) {
            capitulos.put(id, new CapituloImagem(personagens, escaneadorDoArquivo));// aqui ele valores aos dicionario
                                                                                    // se já existisse são substituídos,
                                                                                    // ou seja
                                                                                    // após ler os capítulos ele toma
